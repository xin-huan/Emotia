...
class x {
  ...
  resize() {
    ...
    if (this.#e.size instanceof Object) {
      ...
      e = this.canvas.parentNode.clientWidth;
      t = this.canvas.parentNode.clientHeight;
    } else {
      e = window.innerWidth;
...
function TouchMove(e) {
  if (e.touches.length > 0) {
    ...
    for (const [elem, t] of b) {
      ...
      if (D(rect)) { if (!t.hover) { t.hover = true; t.touching = true; t.onEnter(t); } t.onMove(t); } else if (t.hover && t.touching) { t.onMove(t); }
    }
  }
}
...
function D(e) {
  const { x: t, y: i } = A; const { left: s, top: n, width: o, height: r } = e;
  ...
  update(e) {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      for (let jdx = idx + 1; jdx < t.count; jdx++) {
        const otherBase = 3 * jdx; O.fromArray(s, otherBase); N.fromArray(o, otherBase);
        const otherRadius = n[jdx]; _.copy(O).sub(I); const dist = _.length(); const sumRadius = radius + otherRadius;
        if (dist < sumRadius) {
      ...
      if (t.controlSphere0) {
        ...
        if (dist < sumRadius0) {
          const diff = sumRadius0 - dist; j.copy(_.normalize()).multiplyScalar(diff); H.copy(j).multiplyScalar(Math.max(B.length(), 2));
          I.sub(j); B.sub(H);
        }
      }
  ...
  constructor(e) {
    ...
    this.onBeforeCompile = shader => {
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        void RE_Direct_Scattering(const in IncidentLight directLight, const in vec3 normal, const in vec3 viewDir, inout ReflectedLight reflectedLight) {
          vec3 scatteringHalf = normalize(directLight.direction + (normal * thicknessDistortion));
          float scatteringDot = pow(saturate(dot(viewDir, -scatteringHalf)), thicknessPower) * thicknessScale;

          // 【修复】新版 ThreeJS 中 vColor 是 vec4，强制转换为 .rgb 解决类型不匹配报错
          #ifdef USE_COLOR
      ...
      // 2. 挂载到光照计算核心 (兼容 ThreeJS R130-R165+)
      let chunk = h.lights_fragment_begin;

      // 兼容最新版的 Three.js (使用 geometry struct)
      chunk = chunk.replace(
        /RE_Direct\s*\(\s*directLight\s*,\s*geometry\s*,\s*material\s*,\s*reflectedLight\s*\)\s*;/g,
        `RE_Direct( directLight, geometry, material, reflectedLight );
         RE_Direct_Scattering(directLight, geometry.normal, geometry.viewDir, reflectedLight);`
      );

      // 兼容旧版的 Three.js (散装参数)
      chunk = chunk.replace(
        /RE_Direct\s*\(\s*directLight\s*,\s*geometryPosition\s*,\s*geometryNormal\s*,\s*geometryViewDir\s*,\s*geometryClearcoatNormal\s*,\s*material\s*,\s*reflectedLight\s*\)\s*;/g,
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    const o = new g(1, 32, 32); // 控制面数确保性能
      ...
      const t = (function (e) {
        ...
        return {
          ...
          getColorAt: function (ratio, out = new l()) {
            ...
            const idx = Math.floor(scaled); const start = i[idx];
...
function createBallpit(e, t = {}) {
  ...
  const h = S({
    domElement: e,
    onMove() {
      n.setFromCamera(h.nPosition, i.camera); i.camera.getWorldDirection(o.normal); n.ray.intersectPlane(o, r);
      s.physics.center.copy(r); s.config.controlSphere0 = true;
    },
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  return <canvas className={`absolute inset-0 block w-full h-full outline-none ${className}`} ref={canvasRef} />;
};
