...
function createShader(gl, type, source) { const s = gl.createShader(type); gl.shaderSource(s, source); gl.compileShader(s); if (gl.getShaderParameter(s, gl.COMPILE_STATUS)) return s; gl.deleteShader(s); return null; }

function createProgram(gl, sources, tfVaryings, attrLocs) {
  ...
  if (attrLocs) for (const a in attrLocs) gl.bindAttribLocation(p, attrLocs[a], a);
  gl.linkProgram(p);
...
function makeBuffer(gl, d, u) { const b = gl.createBuffer(); gl.bindBuffer(gl.ARRAY_BUFFER, b); gl.bufferData(gl.ARRAY_BUFFER, d, u); gl.bindBuffer(gl.ARRAY_BUFFER, null); return b; }

function createAndSetupTexture(gl, minF, magF, wS, wT) {
...
class ArcballControl {
  ...
  constructor(canvas, updateCallback) {
    ...
    this.pointerPos = vec2.create(); this.previousPointerPos = vec2.create(); this._rotationVelocity = 0; this._combinedQuat = quat.create();
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      const INTENSITY = 0.3 * timeScale, ANGLE_AMPLIFICATION = 5 / timeScale;
      const mid = vec2.sub(vec2.create(), this.pointerPos, this.previousPointerPos); vec2.scale(mid, mid, INTENSITY);
      if (vec2.sqrLen(mid) > this.EPSILON) {
        ...
        vec2.copy(this.previousPointerPos, mid); angleFactor *= ANGLE_AMPLIFICATION;
      ...
      if (this.snapTargetDirection) {
        const SNAPPING_INTENSITY = 0.2, a = this.snapTargetDirection, b = this.snapDirection;
        const sqrDist = vec3.squaredDistance(a, b), distanceFactor = Math.max(0.1, 1 - sqrDist * 10);
    ...
    const rad = Math.acos(this._combinedQuat[3]) * 2.0, s = Math.sin(rad / 2.0); let rv = 0;
  ...
  quatFromVectors(a, b, out, angleFactor = 1) { const axis = vec3.cross(vec3.create(), a, b); vec3.normalize(axis, axis); const d = Math.max(-1, Math.min(1, vec3.dot(a, b))); quat.setAxisAngle(out, axis, Math.acos(d) * angleFactor); return { q: out, axis, angle: 0 }; }
  #project(pos) { const r = 2, w = this.canvas.clientWidth, h = this.canvas.clientHeight, s = Math.max(w, h) - 1; const x = (2 * pos[0] - w - 1) / s, y = (2 * pos[1] - h - 1) / s; let z = 0; const xySq = x * x + y * y, rSq = r * r; if (xySq <= rSq / 2.0) z = Math.sqrt(rSq - xySq); else z = rSq / Math.sqrt(xySq); return vec3.fromValues(-x, y, z); }
}
...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0) {
    this.canvas = canvas; this.items = items || []; this.onActiveItemChange = onActiveItemChange || (() => {}); this.onMovementChange = onMovementChange || (() => {}); this.scaleFactor = scale; this.camera.position[2] = 3 * scale; this.#init(onInit);
  }
  ...
  #init(onInit) {
    this.gl = this.canvas.getContext('webgl2', { antialias: true, alpha: true, premultipliedAlpha: false }); const gl = this.gl; if (!gl) throw new Error('No WebGL 2 context!');
  ...
  #initDiscInstances(count) {
    ...
    for (let i = 0; i < count; ++i) { const m = new Float32Array(this.discInstances.matricesArray.buffer, i * 16 * 4, 16); m.set(mat4.create()); this.discInstances.matrices.push(m); }
    ...
    const mat4AttribSlotCount = 4, bytesPerMatrix = 16 * 4;
    for (let j = 0; j < mat4AttribSlotCount; ++j) { const loc = this.discLocations.aInstanceMatrix + j; gl.enableVertexAttribArray(loc); gl.vertexAttribPointer(loc, 4, gl.FLOAT, false, bytesPerMatrix, j * 4 * 4); gl.vertexAttribDivisor(loc, 1); }
  ...
  #render() {
    const gl = this.gl; gl.useProgram(this.discProgram); gl.enable(gl.CULL_FACE); gl.enable(gl.DEPTH_TEST); gl.clearColor(0, 0, 0, 0); gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT); gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA); gl.enable(gl.BLEND);
    gl.uniformMatrix4fv(this.discLocations.uWorldMatrix, false, this.worldMatrix); gl.uniformMatrix4fv(this.discLocations.uViewMatrix, false, this.camera.matrices.view); gl.uniformMatrix4fv(this.discLocations.uProjectionMatrix, false, this.camera.matrices.projection);
    ...
    gl.uniform1f(this.discLocations.uFrames, this.#frames); gl.uniform1i(this.discLocations.uTex, 0);
  ...
  #findNearestVertexIndex() {
    const n = this.control.snapDirection; const inversOrientation = quat.conjugate(quat.create(), this.control.orientation); const nt = vec3.transformQuat(vec3.create(), n, inversOrientation);
