...
class Canvas {
  constructor({ container, canvas: canvasEl, items, planeWidth, planeHeight, distortion, scrollEase, cameraFov, cameraZ, onActiveChange }) {
    ...
    if (this.medias && this.medias.length > 0) {
      this.scroll.current = this.medias[0].y;
      this.scroll.target = this.medias[0].y;
    }
  ...
  update() {
    ...
    if (this.medias) {
      ...
      // find media closest to viewport center
      let closestIdx = 0;
      let closestDist = Infinity;
      this.medias.forEach(media => {
        const dist = Math.abs(media.plane.position.y);
        if (dist < closestDist) { closestDist = dist; closestIdx = media.index; }
      });
      if (closestIdx !== this.activeIndex && this.onActiveChange) {
        this.activeIndex = closestIdx;
        this.onActiveChange((closestIdx % this.items.length + this.items.length) % this.items.length);
      }
    }
