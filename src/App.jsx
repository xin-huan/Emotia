...
import { motion, useMotionValue, useTransform, AnimatePresence } from 'motion/react'; // 确保安装了 motion/react 或 framer-motion
import AOS from 'aos';
...
const Navbar = ({ onLoginClick }) => (
  <nav className="fixed top-0 left-0 w-full bg-wysa-green px-12 md:px-24 py-3 flex items-center justify-between z-50 shadow-md">
    <div className="text-3xl font-extrabold text-white font-sans relative z-10">Emotia</div>
    <div className="flex-1 flex justify-center text-sm font-medium relative z-0">
      <GooeyNav items={navItems} animationTime={500} />
    </div>
    <button
      onClick={onLoginClick}
      className="bg-white text-wysa-green px-6 py-1.5 rounded-full shadow-sm hover:bg-wysa-pink transition text-sm font-bold relative z-10 cursor-pointer"
    >
      登录
    </button>
  </nav>
  ...
  <section className="pt-12 pb-16 px-12 md:px-24 w-full border-t-2 border-dashed border-gray-100 bg-wysa-pink relative overflow-hidden">
    ...
    <div className="relative w-full max-w-5xl mx-auto h-[540px] z-10">
      ...
      <div className="absolute top-[62%] left-1/2 -translate-x-1/2 -translate-y-1/2 text-center z-0 w-full flex flex-col items-center">
        ...
        <button className="group border-2 border-wysa-coral text-wysa-coral rounded-full px-8 py-2.5 text-lg font-bold hover:bg-wysa-coral hover:text-white transition-all duration-300 relative z-30 inline-flex items-center space-x-2 shadow-sm">
          <span>了解更多</span><span>→</span>
        </button>
      </div>
    </div>
  ...
  <section className="min-h-screen pt-20 pb-16 px-12 md:px-24 w-full bg-wysa-pink relative flex flex-col justify-center border-t-2 border-dashed border-white/60">
    ...
    <div className="flex flex-col lg:flex-row justify-between items-stretch gap-10 h-[460px] w-full max-w-7xl mx-auto">
      ...
      <PixelTransition
        ...
        firstContent={<img src="/1.png" alt="People" className="w-full h-full object-cover rounded-[40px]" />}
        secondContent={
          <div className="w-full h-full bg-white p-10 flex flex-col justify-center relative overflow-hidden rounded-[40px]">
  ...
  return (
    <section className="py-24 px-12 md:px-24 w-full relative">
      ...
      <div className="w-full h-[350px] rounded-[50px] mb-16 shadow-xl overflow-hidden relative group">
        <img src="/2.jpg" alt="Features Banner" className="w-full h-full object-cover relative z-0" />
    ...
    <section className="py-16 px-12 md:px-24 w-full flex flex-col gap-10">
      ...
      <div className="flex flex-col lg:flex-row gap-8 w-full items-stretch">
        ...
        <div className="w-full lg:w-[45%] flex flex-col">
          ...
          <div className="h-[360px] bg-white/40 backdrop-blur-md border border-wysa-coral/30 rounded-[30px] p-6 flex justify-center items-center relative overflow-hidden shadow-lg group">
            <div className="absolute top-[-10%] left-[-10%] w-40 h-40 bg-wysa-coral/20 rounded-full blur-2xl pointer-events-none"></div>
            <div className="absolute bottom-[-10%] right-[-10%] w-40 h-40 bg-wysa-green/10 rounded-full blur-2xl pointer-events-none"></div>
        ...
        <div className="w-full lg:w-[55%] flex flex-col">
          ...
          <div className="h-[360px] bg-wysa-coral/45 border border-wysa-green rounded-[30px] p-6 flex flex-col relative shadow-lg text-white">
              ...
              {faqData.map((item) => (
                <div key={item.num} className="group flex items-start gap-3 cursor-pointer hover:bg-white/10 py-2 px-2 -mx-2 rounded-xl transition-colors">
                  <span className={`shrink-0 w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center mt-0.5 shadow-sm
                    ${item.num <= 3 ? 'bg-wysa-coral text-white' : 'bg-white/20 text-white/80'}`}>
                    {item.num}
                  </span>
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-white/50 mt-1 flex items-center gap-1">
                      <span className="text-wysa-coral">💬</span> {item.replies} 条深度回复
                    </p>
                  </div>
                </div>
              ))}
...
function App() {
  ...
  return (
    <ClickSpark
      ...
      duration={500}
    >
      <div className="font-sans text-gray-900 min-h-screen w-full overflow-x-hidden antialiased relative">

        {/* 固定背景层 - 确保粉色背景生效 */}
        <div className="fixed inset-0 bg-wysa-pink -z-10"></div>
